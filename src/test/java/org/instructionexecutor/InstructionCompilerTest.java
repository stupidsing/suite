...
public class InstructionCompilerTest {
	...
	public void test() throws IOException, ClassNotFoundException, IllegalAccessException, InstantiationException {
		...
		Node result = execCompiled(code);
		assertEquals(Atom.TRUE, result);
	}
	...
	private Node execCompiled(Node code) throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
		InstructionCompiler instructionCompiler = new InstructionCompiler("/tmp/" + InstructionCompiler.class.getName());
		instructionCompiler.compile(code);

		try (CompiledRun compiledRun = instructionCompiler.getCompiledRun()) {
