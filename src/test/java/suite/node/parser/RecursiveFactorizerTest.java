...
public class RecursiveFactorizerTest {
	...
	private FNode transform(FNode fn) {
		...
		if (fn instanceof FTree) {
			FTree ft = (FTree) fn;
			return new FTree(ft.name, Read.from(ft.fns).map(this::transform).toList());
		} else {
			FTerminal ft = (FTerminal) fn;
			return new FTerminal(ft.pre, ft.chars.replace(from, to), ft.post);
		}
	}
