...
public class RecursiveFactorizerTest {
	...
	public void testRefactorRewrite() throws IOException {
		Source<Node[]> source = () -> {
			...
			return new Node[] { fun.apply("ic-compile-better-option").apply(false),
					fun.apply("ic-new-compile-better-option").apply(true) };
		};
		...
		String sx = recursiveFactorizer.unparse(frx);
		System.out.println(sx);
		assertFalse(sx.contains("ic-compile-better-option"));
	}
