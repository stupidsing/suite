...
public class AssetAllocator_ {
	...
	public static AssetAllocator byTradeFrequency(AssetAllocator assetAllocator, int tradeFrequency) {
		return new AssetAllocator() {
			...
			public OnDate allocate(Map<String, DataSource> dataSourceBySymbol, List<LocalDate> tradeDates) {
				OnDate onDate = assetAllocator.allocate(dataSourceBySymbol, tradeDates);

				return backTestDate0 -> {
					...
					if (!Objects.equals(date0, backTestDate1)) {
						date0 = backTestDate1;
						return result0 = onDate.onDate(backTestDate1);
					} else
						return result0;
				};
			}
	...
	public static AssetAllocator even(AssetAllocator assetAllocator) {
		return (dataSourceBySymbol, tradeDates) -> {
			OnDate onDate = assetAllocator.allocate(dataSourceBySymbol, tradeDates);

			return backTestDate -> {
				List<Pair<String, Double>> potentialBySymbol = onDate.onDate(backTestDate);
