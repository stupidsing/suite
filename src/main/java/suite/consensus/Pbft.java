...
public class Pbft {
	...
	public enum Mt {
		REQUEST, PREP, PREPARE, COMMIT, REPLY, VIEWCHANGE, NEWVIEW,
	}

	public void play() {
		var messages = Map.of(generals.get(0), Map.of(client, new Message(Mt.REQUEST, ".")));
	...
	public class GoodGeneral implements General { // a good one
		...
		public Map<General, Message> round(int epoch, Map<General, Message> recvs) {
			...
			if (prepareTime + maxPrepareDuration < epoch) {
				...
				if (this == leader)
					if (recvs.get(client).type == Mt.REQUEST)
						return Read //
								.from(generals) //
				...
				if (nGenerals - maxTraitors <= count.f(m -> m.isDecision(Mt.COMMIT, decision))) {
					...
					state = State.PRE____;
					return Map.ofEntries(entry(client, new Message(Mt.REPLY, decision)));
				} else
					return Map.ofEntries();
