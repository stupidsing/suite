...
JAVA_HOME=$(cchs "curl -sL https://jdk.java.net/14/" "grep https://download.java.net/ | grep -v sha256 | grep linux | grep -v alpine | grep \\.tar\\.gz" "cut -d\\\" -f2" @curl @tar-zxf @dir)
