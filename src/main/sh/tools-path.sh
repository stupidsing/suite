...
GOROOT=$(cchs "echo https://dl.google.com/go/go1.23.4.linux-amd64.tar.gz" @curl @tar-zxf @dir)
GRADLE_HOME=$(cchs "echo https://services.gradle.org/distributions/gradle-8.12-bin.zip" @curl @unzip @dir)
JAVA_HOME=$(cchs "curl -sL https://jdk.java.net/23/" "grep https://download.java.net/ | grep -v sha256 | grep linux-x64 | grep \\.tar\\.gz" "cut -d\\\" -f2" @curl @tar-zxf @dir)
